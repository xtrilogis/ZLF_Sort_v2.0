...
from assethandling.basemodels import File, FileType, ExcelConfig
                ...
                if b.is_dir():
                    for c in b.iterdir():
                        if c.is_file(): # tatsächliche files
                            _add_file_object(file=c, all_files=structure[a.name][b.name],
                                             progress_callback=progress_callback)
                        if c.is_dir():
...
def _get_all_files(raw_material_folder: Path, progress_callback) -> List[File]:
    ...
    for element in raw_material_folder.glob('**/*'):
        _add_file_object(file=element, all_files=all_files, progress_callback=progress_callback)
    return all_files
...
def fill_excel(excel: Path, raw_material_folder: Path, progress_callback):
    ...
    for element in raw_material_folder.glob('**/*'):
        if _is_folder_with_material(element):
            _fill_sheets(folder=element, sheets=sheets, progress_callback=progress_callback)
...
def _fill_sheets(folder: Path, sheets: Dict[str, pd.DataFrame], progress_callback):
    ...
    for element in folder.iterdir():
        try:
            ...
            progress_callback.emit(f"Fehler: Datei {element.name} nicht in der Excel-Datei gefunden.")


def create_picture_folder(picture_folder: Path, raw_material_folder: Path, progress_callback):
    ...
    for element in raw_material_folder.glob('**/*'):
        if element.suffix.upper() in constants.image_extensions:
            try:
                ...
                progress_callback.emit(f"Datei {element.name} wurde nicht umbenannt\n"
