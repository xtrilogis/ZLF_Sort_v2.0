...
def test_folder_creation(mock_create_folder, test_data_path):
    date = datetime.now()
    mock_create_folder.return_value = test_data_path / f'Zeltlagerfilm {date.year}'

    create_folder_structure(test_data_path, date)
    assert mock_create_folder.called
    assert mock_create_folder.call_count == 31  # wegen 31 Ordnern
    assert mock_create_folder.call_args_list[0].kwargs['parent'] == test_data_path
