...
from fileopertations.filemethods import create_folder, copy_file, \
    rename_files, get_file_type, get_file_captured_date
...
def test_copy_file(mock_copy, testdata_path, dummy_file):
    ...
    assert mock_copy.call_args.kwargs["dst"] == testdata_path / "dummy folder/dummy_file.txt"
...
def test_rename_files(mock_rename, testdata_path):
    ...
    files = [File(full_path=testdata_path / "/Rohmaterial/a 27.07. Mi/Videos/07-27-Mi_001.MP4",
             ...
             File(full_path=testdata_path / "/Rohmaterial/a 27.07. Mi/Videos/07-27-Mi_002.AVI",
                  date=datetime(2022, 7, 27, 12, 31), dst_folder=None, type=FileType.VIDEO),
             File(full_path=testdata_path / "/Rohmaterial/a 27.07. Mi/Videos/07-27-Mi_003.AVI",
                  date=datetime(2022, 7, 27, 12, 31), dst_folder=None, type=FileType.VIDEO)
             ]
...
def test_get_captured_date(testdata_path):
    ...
    assert get_file_captured_date(path / "07-28-Fr_003.MOV", FileType.VIDEO) == \
           datetime(2023, 7, 28, 12, 36, 38)
    assert get_file_captured_date(path / "Sonstiges_028.MOV", FileType.VIDEO) == \
           datetime(2023, 8, 5, 15, 15, 22)
    assert get_file_captured_date(path / "07-28-Fr_229.JPG", FileType.IMAGE) == \
           datetime(2023, 7, 28, 21, 22, 36)
    assert get_file_captured_date(path / "07-28-Fr_230.JPG", FileType.IMAGE) == \
           datetime(2023, 7, 28, 14, 5, 10)
    assert get_file_captured_date(path / "08-05-Sa_003.JPG", FileType.IMAGE) == \
